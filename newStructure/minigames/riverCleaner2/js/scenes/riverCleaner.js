...
var riverCleaner = function(){
    ...
    function createTrashMeter(){
        ...
        var containter = meterGroup.create(game.world.centerX, game.world.height - 80 , 'atlas.riverCleaner', "container")
        containter.anchor.setTo(0.5)
        containter.scale.setTo(1.7)
        
        var bar = meterGroup.create(containter.x - containter.width * 0.28, containter.y, 'atlas.riverCleaner', "bar")
        bar.scale.setTo(0, 2.5)
    ...
    function createPolution(){
        ...
        var cloud = new Phaser.Graphics(game)
        cloud.beginFill(0x004400)
        cloud.drawRect(0,0,game.world.width, game.world.height)
        cloud.endFill()
        cloud.alpha = 0
        cloud.alive = false
