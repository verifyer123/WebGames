...
var galactic = function(){
	...
	function onClickPlay(){
		...
		for(var loadPlanets=0;loadPlanets<9;loadPlanets++)
		{
	...
	function reset()
	{
		...
		dragablePlanets[0].inputEnabled=false;
	...
	function update(){
		...
		if(startTime==false && timetoHit>300)
		{
			for(var positionNebul=1; positionNebul<9; positionNebul++)
			{
				...
				if(dificultyInLevel<9)
				{
					...
					nebul[dificultyInLevel].alpha=0.7;
				}
			}
		...
		if(timetoHit>300)
		{
			...
			for(var canDrag=0; canDrag<9;canDrag++)
			{
