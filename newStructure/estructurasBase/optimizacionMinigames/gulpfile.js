...
var cloneFiles = function(folder){
  var conditions = [scriptsPath+"/"+folder+'/**/*'];
  conditions.push('!minigames/**/dist');
  for (var i = 0; i < _FILES.CLONE_EXCEPTIONS.length; i++) {
    conditions.push("!"+scriptsPath+"/"+folder+"/"+_FILES.CLONE_EXCEPTIONS[i]);
    console.log("!"+scriptsPath+"/"+folder+"/"+_FILES.CLONE_EXCEPTIONS[i]);
  }
  return gulp.src(conditions)
    .pipe(rename(function(path){
      console.log(path);
    }))
    .pipe(gulp.dest(scriptsPath+"/"+folder+"/"+_ROUTES.DIST.HTML));
}
...
var prepareJsToDist = function(folder, unique){
  folder = folder || "";
  unique = unique || false;
  var conditions = [scriptsPath+"/"+folder+"/"+'*.js'];
  for (var i = 0; i < _FILES.JS_EXCEPTIONS.length; i++) {
    conditions.push("!"+scriptsPath+"/"+folder+"/"+_FILES.JS_EXCEPTIONS[i]);
    console.log("!"+scriptsPath+"/"+folder+"/"+_FILES.JS_EXCEPTIONS[i]);
  }
  if (unique) {
    //ejecucion para todos los archivos
    console.log(scriptsPath+"/"+folder+"/"+'*.js');
    return gulp.src(conditions)
    ...
    return gulp.src(scriptsPath+"/"+folder+"/"+'js/**/*.js')
      .pipe(gulpif(isMinified, obfuscate(_OBFUSCATE_OPT)))
      .pipe(concat(_ROUTES.MIN_JS.NAME+'.js'))
      .pipe(rename(_ROUTES.MIN_JS.NAME+'.'+_ROUTES.MIN_JS.SUFIX+'.js'))
      .pipe(gulp.dest(scriptsPath+"/"+folder+"/"+_ROUTES.DIST.JS));
  }
}
...
var changeHtmlReference = function(folder){
  return gulp.src(scriptsPath+"/"+folder+"/"+'index.html')
...
// Carpeta sobre la que se está trabajando task principal
var scriptsPath = 'minigames';
