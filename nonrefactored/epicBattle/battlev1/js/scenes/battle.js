...
1
	...
	function createGameObjects(){
		...
		player1.numPlayer = 1
		player2 = createPlayer(serverData.p2, {x:BATTLE_WIDTH - WIDTH_DISTANCE, y: game.world.height - 150}, -1)
		player2.numPlayer = 2
    ...
    return {
        ...
        preload:preload,
		update:function () {
			leftSide.renderXY(sceneGroup, sceneGroup.x - VIEW_WIDTH * 2, -250, true);
		},
        create: function(event){

        	sceneGroup = game.add.group()
			sceneGroup.x = game.world.centerX - VIEW_WIDTH * 0.5
			leftSide = game.add.renderTexture(game.world.width, game.world.height, 'texture1');

			var poly = new Phaser.Polygon();
			poly.setTo([ new Phaser.Point(VIEW_WIDTH, 0), new Phaser.Point(VIEW_WIDTH, game.world.height), new Phaser.Point(0, 150), new Phaser.Point(0, 0)]);

        	var mask = game.add.graphics(game.world.centerX - VIEW_WIDTH * 0.5, 0);
			mask.beginFill(0xffffff);
			mask.drawPolygon(poly.points);
			// sceneGroup.add(mask)

			var secondScreen = game.add.sprite(0, 0, leftSide)
			secondScreen.mask = mask;
			// game.add.sprite(0, 0, rightSide)
			// rightSide.renderXY(sceneGroup, 0, 0, true);
