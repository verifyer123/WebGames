...
var bax = function(){
    ...
    var assets = {
        atlases: [
            {
                name: "atlas.bax",
                json: "images/bax/atlas.json",
                image: "images/bax/atlas.png"
            }
        ],
        images: [
            // {   name:"fondo",
	...
	function blowAttack(asset, from, target, scale, color, onComplete){
		...
		var proyectile = sceneGroup.create(0, 0, 'atlas.bax', asset)
		proyectile.x = from.x
		proyectile.y = from.y
    ...
    function hideQuestion(){
        game.add.tween(baxGroup.number1).to({alpha:0}, 800, Phaser.Easing.Cubic.Out, true)
        game.add.tween(baxGroup.operator).to({alpha:0}, 800, Phaser.Easing.Cubic.Out, true)
        game.add.tween(baxGroup.number2).to({alpha:0}, 800, Phaser.Easing.Cubic.Out, true)

        for(var optionIndex = 0; optionIndex < baxGroup.options.length; optionIndex++) {
            var option = baxGroup.options[optionIndex]
    ...
    function checkAnswer(circle) {
        ...
        if(inputsEnabled){
            ...
            if (option.number === baxGroup.answer) {
                var particleCorrect = baxGroup.correctParticle
                ...
                var particleWrong = baxGroup.wrongParticle
	...
	function createHpbar(){
		...
		var container = hpGroup.create(0, -22, 'atlas.bax', 'healthcontainer')
		container.anchor.setTo(0.5, 0.5)

		var hpBg = sceneGroup.create(0, 0, 'atlas.bax', 'hp_bar')
    ...
    function createGameObjects(){
        ...
        var floor = sceneGroup.create(0, 0, 'atlas.bax', 'floor')
        ...
        var floor2 = sceneGroup.create(0, 0, 'atlas.bax', 'floor')
    ...
    function createPart(key){
        ...
        particle.makeParticles('atlas.bax',key);
	...
	function createPointsBar(){
		...
		var pointsImg = pointsBar.create(-10,10,'atlas.bax','xpcoins')
    ...
    function preload(){
        ...
        game.load.audio('baxSong', soundsPath + 'songs/mysterious_garden.mp3');

        game.load.image('introscreen',"images/bax/introscreen.png")
        game.load.image('howTo',"images/bax/how" + localization.getLanguage() + ".png")
        game.load.image('buttonText',"images/bax/play" + localization.getLanguage() + ".png")
    ...
    function numbersEffect() {
        ...
        baxGroup.number1.alpha = 0
        baxGroup.operator.alpha = 0
        baxGroup.number2.alpha = 0

        baxGroup.number1.scale.x = 0.2
        baxGroup.number1.scale.y = 0.2
        baxGroup.operator.scale.x = 0.2
        baxGroup.operator.scale.y = 0.2
        baxGroup.number2.scale.x = 0.2
        baxGroup.number2.scale.y = 0.2

        game.add.tween(baxGroup.number1.scale).to({x: 1,y:1}, 800, Phaser.Easing.Bounce.Out, true)
        game.add.tween(baxGroup.operator.scale).to({x: 1,y:1}, 800, Phaser.Easing.Bounce.Out, true)
        game.add.tween(baxGroup.number2.scale).to({x: 1,y:1}, 800, Phaser.Easing.Bounce.Out, true)
        game.add.tween(baxGroup.number1).to({alpha:1}, 800, Phaser.Easing.Cubic.Out, true)
        game.add.tween(baxGroup.operator).to({alpha:1}, 800, Phaser.Easing.Cubic.Out, true)
        game.add.tween(baxGroup.number2).to({alpha:1}, 800, Phaser.Easing.Cubic.Out, true)
    }
    ...
    function generateQuestion() {
        ...
        baxGroup.answer = answer
        baxGroup.number1.setText(number1)
        baxGroup.operator.setText(operation)
        baxGroup.number2.setText(number2)
        numbersEffect()
    ...
    function createBaxUI() {

        baxGroup = game.add.group()
        baxGroup.x = game.world.centerX
        baxGroup.y = game.height - 200
        sceneGroup.add(baxGroup)
        ...
        colorBox.y = -200
        baxGroup.add(colorBox)

        var dashboad = baxGroup.create(0, 0, 'atlas.bax', 'UIcontainer')
        ...
        for(var optionIndex = 0; optionIndex < NUM_OPTIONS; optionIndex++){
            ...
            var optionCircle = option.create(0, 0, 'atlas.bax', 'option')
        ...
        baxGroup.number2 = number2

        var correctParticle = createPart("star")
        baxGroup.add(correctParticle)
        baxGroup.correctParticle = correctParticle

        var wrongParticle = createPart("wrong")
        baxGroup.add(wrongParticle)
        baxGroup.wrongParticle = wrongParticle
    ...
    function createTutorial(){
        ...
        var tuto = tutoGroup.create(game.world.centerX, game.world.centerY - 50,'atlas.bax','gametuto')
        ...
        var inputLogo = tutoGroup.create(game.world.centerX ,game.world.centerY + 125,'atlas.bax',inputName)
        inputLogo.anchor.setTo(0.5,0.5)
        inputLogo.scale.setTo(0.7,0.7)

        var button = tutoGroup.create(game.world.centerX, inputLogo.y + inputLogo.height * 1.5,'atlas.bax','button')
    ...
    return {
        ...
        create: function(event){
			...
			var wall = game.add.tileSprite(0,0,game.world.width, 320, 'atlas.bax','wall')
			sceneGroup.add(wall)

            baxSong = game.add.audio('baxSong')
            game.sound.setDecodedCallback(baxSong, function(){
                baxSong.loopFull(0.6)
            }, this);
            ...
            initialize()
			var window = sceneGroup.create(game.world.centerX,game.world.centerY - 320, "atlas.bax", "window")
			window.anchor.setTo(0.5, 0.5)

			var pillar = sceneGroup.create(game.world.centerX - 150,game.world.centerY - 320, "atlas.bax", "pillar")
			pillar.anchor.setTo(0.5, 0.5)

			var pillar2 = sceneGroup.create(game.world.centerX + 150,game.world.centerY - 320, "atlas.bax", "pillar")
            ...
            buttons.getButton(baxSong,sceneGroup)
