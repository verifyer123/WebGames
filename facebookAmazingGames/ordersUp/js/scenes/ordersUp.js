...
var ordersUp = function(){
    ...
    var BOSS_PROBABILIY = 0.6
    var PEOPLE_TO_BOSS = 8
    ...
    function initialize(){
        ...
        currentLessDifficult = DELTA_LESS_DIFFICULT
    ...
    function preload() {
        ...
        marioSong = sound.setSong(soundsPath + 'jungle_fun.mp3',1)
    ...
    function setSecuence(isBoos){
        ...
        var difficultRounded
        //if(currentDifficult < 3){
          //  difficultRounded = 2
        //}
        //else{
            difficultRounded = Math.floor(currentDifficult)
        //}

        if(isBoos){
            currentDifficult -= currentLessDifficult
            currentLessDifficult+=0.2
    ...
    function setButtonGroup(group,idProduct){
        ...
        var idsUsed = []
        var multiplyIndex = 0
        if(PRODUCT_TYPES_ARRAYS[idProduct-1].length >= MAX_PRODUCTS_PER_TYPE*2){
            multiplyIndex = game.rnd.integerInRange(0,1)
        }
        for(var i = 1; i<=MAX_PRODUCTS_PER_TYPE; i++){
            var r = multiplyIndex*MAX_PRODUCTS_PER_TYPE

            if(idsUsed.indexOf(PRODUCT_TYPES_ARRAYS[idProduct-1][r])!=-1){
    ...
    function createBackground(){
        ...
        sceneGroup.add(currentButtonGroup)
        sceneGroup.add(secondButtonGroup)
    }
