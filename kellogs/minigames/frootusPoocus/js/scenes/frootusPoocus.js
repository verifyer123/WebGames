...
var frootusPoocus = function(){
	...
	assets = {
		atlases: [
			{   
				name: "atlas.frootusPoocus",
				json: "images/frootusPoocus/atlas.json",
				image: "images/frootusPoocus/atlas.png",
			},
		...
		images: [
			{
				name: "dock",
				file: "images/frootusPoocus/dock.png",
			},
			{
				name:'tutorial_image',
				file:"images/frootusPoocus/tutorial_image.png"
			}
		],
		sounds: [
			{	name: "magic",
	...
	function createPointsBar(){
		...
		var pointsImg = pointsBar.create(-10,10,'atlas.frootusPoocus','xpcoins')
	...
	function createHearts(){
		...
		var heartImg = group.create(0,0,'atlas.frootusPoocus','life_box')
	...
	function createBackground(){
		...
		var back = sceneGroup.create(0, 0, "atlas.frootusPoocus", "background");
		back.width = game.world.width
		back.height = game.world.height

		weves = game.add.tileSprite(0, 0, game.world.width * 2, game.world.height, "atlas.frootusPoocus", 'wave')
		...
		var tree1 = sceneGroup.create(game.world.centerX - dock.width * 0.58, 2, "atlas.frootusPoocus",  "tree1")
		var tree2 = sceneGroup.create(game.world.centerX + dock.width * 0.35, 2, "atlas.frootusPoocus",  "tree2")
		var tree3 = sceneGroup.create(game.world.centerX + dock.width * 0.36, dock.y - 150, "atlas.frootusPoocus",  "tree3")
	...
	function createPart(key){
		...
		particle.makeParticles('atlas.frootusPoocus', key);
	...
	function createWitch(){
		...
		var box = sceneGroup.create(game.world.centerX, 150, 'atlas.frootusPoocus', "background");
	...
	function createGems(){
		...
		for(var i = 0; i < 4; i ++){
			...
			var gem = gemsGroup.create(game.world.centerX * pivotX, witch.y + pivotY, "atlas.frootusPoocus", "gem" + i)
	...
	function createEnemies(){
		...
		for(var i = 0; i < 4; i++){
			...
			var box = enemiesGroup.create(0, 0, "atlas.frootusPoocus", "body")
			...
			var shield = game.add.sprite(0, -120, "atlas.frootusPoocus", "broken" + i)
	...
	function setDamage(mask){
		...
		switch(mask.healtPoints){
			...
			case 0:
				...
				case 2:
				sound.play("glassbreak")
				mask.shield.loadTexture("atlas.frootusPoocus", "broken" + mask.element)
				game.add.tween(mask).to({y: mask.y + 100}, 500, Phaser.Easing.Cubic.Out, true).onComplete.add(function(){
					mask.body.velocity.y = -speed
					mask.touch = false
					witch.canAttack = true;
		...
		for(var enemysToRelease=0; enemysToRelease<enemysInLine; enemysToRelease++){
			...
			if(mask){
				if(level === 2){
					...
					mask.shield.loadTexture("atlas.frootusPoocus", "shield" + mask.element)
					mask.shield.alpha = 1
				}
