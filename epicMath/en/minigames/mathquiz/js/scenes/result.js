var resultScreen = function(){
	...
	function setScore(score, scoreGoal, timeScore){
        ...
        mixpanel.track(
            "finishGame",
            {"gameName": "mathQuiz", "time":totalTime}
        );
	}
	...
	function createSpeedometer(){
		...
		var totalSpeed = new Phaser.Text(game, 0, 0, localization.getString(localizationData, "yourSpeed")+(10 - totalTime)+localization.getString(localizationData, "secPerAnswer"), fontStyle)
		totalSpeed.anchor.setTo(0.5, 0.5)
		totalSpeed.x = background.x 
		totalSpeed.y = background.y + (background.height * 0.79)

		speedometerGroup.add(totalSpeed)

		speedometerGroup.setScore = function(scoreRange){
			//scoreRange = scoreRange >= 1 ? 1 : scoreRange
            scoreRange = totalTime / 10
	...
	function createScene(){
		...
		scoreSprite.x = timerSprite.x + timerSprite.width * 1.2
